...
license: MIT
summary: "A removable-chips token input that tokenizes typed entries into Badge chips with per-kind validation; used by invitation and messaging features to collect recipients."
platforms:
  - typescript
  - web
tags:
...
A removable-chips / token input in `@agenticdevelopertoolkit/ui`. None exists today. It is
used by features that collect a list of typed recipients, e.g. email + SMS
invitation boxes.
...
`@agenticdevelopertoolkit/ui/components/recipient-input`
...
feature (e.g. an invitation-sending modal), not by the input.
