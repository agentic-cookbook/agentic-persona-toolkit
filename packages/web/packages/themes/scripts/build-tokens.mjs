...
// are inputs to this package's bundle, and a staleness guard in a consuming application's
// CI reads "a source file newer than the newest thing in dist/" as "this dist is a
// rollback" — the guard that once caught a status container shipped with a dead ESM
// SyntaxError. That guard runs this script (via `check:drift`) AFTER vendoring dist/, so
// an unconditional rewrite made the vendored-freshness check fail on a tree where not one
// byte had drifted.
//
