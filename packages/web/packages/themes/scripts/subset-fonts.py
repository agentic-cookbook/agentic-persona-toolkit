...
# notice.
#
# The census behind the choice is a script in the consuming application that walks
# every source and content file it ships and counts non-ASCII codepoints by block. It
# lives there rather than here because the corpus is that application's, not this
# package's. Re-run it if the family starts setting type in something new; the usage
# counts below are from that scan (3,691 files, 250 distinct non-ASCII codepoints).
#
...
# static scan of the repo can see and no reviewer can bound. That text is what EXT covers.
CORE_UNICODES = ",".join(
    [
