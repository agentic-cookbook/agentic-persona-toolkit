...
//
// The derivation cases below are lifted from the consuming application's server-side test
// suite's expectations for `deriveTitle` ON PURPOSE: this file is a MIRROR of the server-side
// derivation in the consuming application, and the pair has no shared package to live in
// (separate deployables). Keeping the cases
...
// `firstNonEmpty(frontmatter?.title, frontmatter?.name)` (the server-side derivation in the
// consuming application) — `title` always wins over `name`, regardless of which key comes
// first in the file. And the
