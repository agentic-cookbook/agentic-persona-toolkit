...
export function morphPath(a: ParsedPath, b: ParsedPath, t: number): ParsedPath {
  ...
  // overshooting ease is exactly where that shows: `startled` is `back.out(3)`,
  // whose eased value crosses 1 a quarter of the way through the tween and
  // stays above it for the remaining three quarters. The original animates the
  // polyline's coordinates as ordinary numbers, so the mouth swings past its
  // target and settles back; a clamp here parks it on the target instead and
  // deletes the pop -- 3.16 design units on `startled`, 2.26 on `surprised`,
  // and a visibly softer mouth on every `back.out` pose.
  //
