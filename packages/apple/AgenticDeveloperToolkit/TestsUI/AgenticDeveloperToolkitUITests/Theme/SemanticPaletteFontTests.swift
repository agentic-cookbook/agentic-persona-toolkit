...
/// Relocated from AgenticToolkit's `Tests/AgenticToolkitCoreMacOSTests/Theme/`.
/// It tests the AppKit `NSColor`/`NSFont` bridging (`SemanticPalette+NSColor.swift`,
/// `ThemeTypography+NSFont.swift`), which moved here in the same step — its old
/// home stopped compiling it once `AgenticToolkitCoreMacOS` no longer carried
